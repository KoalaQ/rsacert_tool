...
public class DESForJava {
     ...
     * @throws UnsupportedEncodingException
     */
    public static byte[] encryption(String msg, String key, String charsetName) throws UnsupportedEncodingException {
        //[padding模式]:明文长度,不是8的倍数的,需要用空格补齐
		/*if(msg.getBytes().length%8!=0){
			msg = msg + "        ".substring(msg.getBytes().length%8);
		}*/
        byte[] ret = reportdes(msg.getBytes(charsetName), key.getBytes(), msg.getBytes(charsetName).length, 0);
        return ret;
    }
     ...
     * @throws UnsupportedEncodingException
     */
    public static String decryption(byte[] msg, String key, String charsetName) throws UnsupportedEncodingException {
        byte[] ret = reportdes(msg, key.getBytes(), msg.length, 1);
        return (new String(ret,charsetName)).trim();
    }
